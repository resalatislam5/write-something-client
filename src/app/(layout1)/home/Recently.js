...
function Recently({ itemsPerPage }) {
  ...
  const { handleLikes, handleDislikes, handleBookmark, user } =
    useContext(AuthContext);
  // pagination
  const RecentlyData = async () => {
    const res = await fetch(
      `https://write-something-server.vercel.app/home/recently?page=${postCountStart}`
    );
    ...
    <section>
      <div className="flex gap-5 sm:py-20 py-10">
        <div className="md:w-[60%] w-full">
          <h1 className="my-5">
            <span className="text-dark-cyan text-xl font-semibold p-2 ">
              Recently
            </span>{" "}
            Posted
          </h1>
          {pagePost.length !== 0 ? (
            <div className="flex flex-wrap gap-5 justify-center items-center">
              {pagePost.map((e) => (
                <div
                  key={e}
                  className="flex flex-col lg:gap-5 sm:gap-2 gap-1 sm:p-5 p-2 w-80"
                >
                  <div>
                    <Image
                      src={e.thumbnail}
                  ...
                  <div className="flex flex-col justify-center gap-2 text-[#777]">
                    <p>
                      <span className="bg-cool-mint">{e.tags[0]}</span>
                    </p>
                    <Link
                      href={`/post-details/${e._id}`}
                      className="font-semibold lg:text-2xl sm:text-xl text-sm text-black hover:text-dark-cyan"
                    >
                      {e.title.length < 50
                        ? e.title
                        : e.title.slice(0, 50) + "...."}
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      ...
                      <div className=" flex items-center justify-center">
                        <div className="w-[1px] h-3 bg-davy-gray"></div>
                      </div>
                      ...
                      <div className="flex gap-2 items-center justify-center">
                        {e.likes.includes(user?._id) ? (
                          <button
                            ...
                            onClick={() => handleLikes(e._id, RecentlyData)}
                          >
                            <FcLike />
                          </button>
                        ) : (
                        ...
                        {e.dislikes.includes(user?._id) ? (
                          <button
                            ...
                            onClick={() => handleDislikes(e._id, RecentlyData)}
                          >
                            <RiDislikeFill />
                          </button>
                        ) : (
                        ...
                        {e.bookmark.includes(user?._id) ? (
                          <button
                            className="cursor-pointer text-dark-cyan"
                            onClick={() => handleBookmark(e._id, RecentlyData)}
                          >
            ...
            <h1 className="text-3xl font-semibold text-dark-cyan flex justify-center items-center">
              No Post Avabile
            </h1>
          )}
        </div>
      ...
      <div className="coustom-pagination pb-10">
        <PaginatedItems
          itemsPerPage={10}
          setPostCountStart={setPostCountStart}
          setPostCountEnd={setPostCountEnd}
        />
      </div>
    </section>
  );
